              ...
              <div className="h-48 w-full bg-pink-50 rounded-3xl p-4 border-2 border-pink-100">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    ...
                    <XAxis dataKey="day" fontSize={10} stroke="#db2777" />
                    <YAxis fontSize={10} stroke="#db2777" />
                    <Tooltip contentStyle={{borderRadius: '15px', border: 'none', boxShadow: '0 10px 15px -3px rgba(244, 114, 182, 0.3)'}} />
                    <Line type="monotone" dataKey="amount" stroke="#db2777" strokeWidth={4} dot={{ r: 4, fill: '#db2777' }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="space-y-3">
                {items.map(item => (
                  <div key={item.id} className="flex justify-between items-center p-5 bg-white rounded-2xl border-2 border-pink-100 shadow-sm hover:border-pink-300 transition">
                    <div>
                      <p className="font-bold text-pink-950 text-sm">{item.title}</p>
                      <p className="text-[10px] text-pink-400">{new Date(item.created_at).toLocaleDateString()}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`font-black ${item.amount >= 0 ? 'text-pink-500' : 'text-pink-700'}`}>
                        {item.amount > 0 ? '+' : ''}{item.amount.toLocaleString()}
                      </span>
                      <button onClick={() => deleteItem(item.id)} className="text-pink-200 hover:text-pink-600 transition">✕</button>
                    </div>
                  </div>
                ))}
              </div>
